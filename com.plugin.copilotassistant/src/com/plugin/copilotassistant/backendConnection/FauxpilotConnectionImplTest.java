...
class FauxpilotConnectionImplTest {
	...
	void testParseResponse() {
		...
		try {
			var parsed = new ObjectMapper()
				      .readValue(response, FauxpilotResponse.class);
	...
	void testRequestResponse() {
		try {
			...
			var port = 5000;
			FauxpilotConnectionImpl conn = new FauxpilotConnectionImpl(ip, port);
			var response  = conn.getResponse("what does the ").thenApply(HttpResponse::body).get();
			var parsed = new ObjectMapper()
				      .readValue(response, FauxpilotResponse.class);
			System.out.println(MessageFormat.format("Parsed response: {0}", parsed));
		} catch (JsonProcessingException | InterruptedException | ExecutionException e) {
			e.printStackTrace();
