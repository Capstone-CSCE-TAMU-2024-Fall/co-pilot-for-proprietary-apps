...
public class CodeInsertHandlerTest {
	...
	public void setUp() {
		...
		Mockito.when(editor.getDocumentProvider().getDocument(ArgumentMatchers.any())).thenReturn(document);
	...
	public void testCodeInsertionWhenDisabled() throws Exception {
		// Arrange
		Mockito.when(selection.getOffset()).thenReturn(0);
		Mockito.when(preferenceStore.getBoolean("ENABLE_INSERTION")).thenReturn(false); // Simulate toggle being
																						// disabled
